...
function PortSetup() {
  ...
  const upsertSetup = () => {
    if (!!isEdit) {
      ...
      UPDATE(`/api/Setup/UpdatePortSetup`, body)
        .then(res => {
          if (res.status == 200) {
            openNotificationWithIcon(
              "success",
              "Port Setup",
              "Port Setup Updated Successfully"
            )
            setOpen(false)
            setCheckExpense(false)
            setName("")
      ...
      CREATE("/api/Setup/CreatePortSetup", body)
        .then(res => {
          if (res.status == 200) {
            openNotificationWithIcon(
              "success",
              "Port Setup",
              "Port Setup Added Successfully"
            )
            setOpen(false)
            setCheckExpense(false)
            setName("")
  ...
  const editSetup = item => {
    ...
    setComboText(item.citySetup ? item.citySetup.name : null)
  }
  ...
  return (
    <React.Fragment>
      ...
      <div className="page-content">
        <Container fluid>
          {/* Render Breadcrumb */}
          <Breadcrumbs title="Admin" breadcrumbItem="Port Setup" />
          <BackDrop open={show} />
                          ...
                          <TableBody>
                            {(rowsPerPage > 0
                              ...
                              <TableRow key={i}>
                                ...
                                <TableCell>
                                  {row.citySetup ? row.citySetup.name : "-"}
                                </TableCell>
                                <TableCell>
                                  <Button
        ...
        >
          <DialogTitle id="max-width-dialog-title">Port Setup</DialogTitle>
          <DialogContent>
            <DialogContentText>
              <Form>
                <Row form>
