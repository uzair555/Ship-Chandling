...
import { notification, DatePicker, Space } from "antd"
...
export default function AccountingYear() {
  ...
  var toto1 = moment(fromDate).format("yy-MM-DD")
  var toto2 = moment(toDate).format("yy-MM-DD")

  //  ========================= Api Integration Section Starts =========================

  const getSetups = () => {
    setshow(true)
    GET("/api/Setup/GetAllDateSetup")
      .then(setups => {
  ...
  const upsertSetup = () => {
    if (!!isEdit) {
      ...
      UPDATE(`/api/Setup/UpdateDateSetup`, body)
        .then(res => {
          if (res.status == 200) {
            openNotificationWithIcon(
              "success",
              "Accounting Setup",
              "Accounting Setup Updated Successfully"
            )
            setOpen(false)
            setFromDate(new Date())
            setToDate(new Date())
      ...
      CREATE("/api/Setup/CreateDateSetup", body)
        .then(res => {
          if (res.status == 200) {
            openNotificationWithIcon(
              "success",
              "Accounting Setup",
              "Accounting Setup Added Successfully"
            )
            setOpen(false)
            setFromDate(new Date())
            setToDate(new Date())
  ...
  const editSetup = item => {
    ...
    setSetupId(item.id)
    setFromDate(item.dateFrom)
    setToDate(item.dateTo)
    setOpen(true)
  ...
  return (
    <React.Fragment>
      ...
      <div className="page-content">
        <Container fluid>
          {/* Render Breadcrumb */}
          <Breadcrumbs title="Admin" breadcrumbItem="Accounting Year" />
          <BackDrop open={show} />
                        ...
                        >
                          ...
                          <TableBody>
                            {(rowsPerPage > 0
                              ...
                              <TableRow key={i}>
                                <TableCell>
                                  {moment(row.dateFrom).format("DD-MM-yy")}
                                </TableCell>
                                <TableCell>
                                  {moment(row.dateTo).format("DD-MM-yy")}
                                </TableCell>
                                <TableCell>
                                  <Button
