...
export default function SetupType() {
  ...
  const getVesselSetup = () => {
    setshow(true)
    GET("/api/Setup/GetAllVesselSetup")
      .then(setups => {
  ...
  const upsertVesselSetup = () => {
    if (!!isEdit) {
      ...
      UPDATE(`/api/Setup/UpdateVesselSetup`, body)
        .then(res => {
          if (res.status == 200) {
            openNotificationWithIcon(
              "success",
              "Vessel Setup",
              "Vessel Setup Updated Successfully"
            )
            setOpen(false)
            setName("")
            setIsEdit(false)
      ...
      CREATE("/api/Setup/CreateVesselSetup", body)
        .then(res => {
          if (res.status == 200) {
            openNotificationWithIcon(
              "success",
              "Vessel Setup",
              "Vessel Setup Added Successfully"
            )
            setOpen(false)
            setName("")
            setIsEdit(false)
  ...
  return (
    <React.Fragment>
      ...
      <div className="page-content">
        <Container fluid>
          {/* Render Breadcrumb */}
          <Breadcrumbs title="Admin" breadcrumbItem="Vessel Setup" />
          <BackDrop open={show} />
          <Row>
            <Col md="2"></Col>
            <Col lg="10">
        ...
        >
          <DialogTitle id="max-width-dialog-title">Vessel Setup</DialogTitle>
          <DialogContent>
            <DialogContentText>
              <Form>
                <Row form>
                  ...
                  <Col md={6}>
                    <FormGroup>
                      <Label for="exampleEmail">Vessel Setup</Label>
                      <Input
                        type="text"
                        value={Name}
                        placeholder="Vessel Setup"
