...
function ExpenceSetup() {
  ...
  const getSetupFillCombo = () => {
    setshow(true)
    GET("/api/Setup/GetAccFile3Combo")
      .then(setups => {
  ...
  const getSetups = () => {
    setshow(true)
    GET("/api/Setup/GetAllExpenseSetup")
      .then(setups => {
        if (setups.status == 200) {
          setRows(setups.data)
          setshow(false)
  ...
  const upsertSetup = () => {
    if (!!isEdit) {
      let body = {
        ...
        chkExpense: checkExpense,
        isValid: true,
      }
      UPDATE(`/api/Setup/UpdateExpenseSetup`, body)
        .then(res => {
          if (res.status == 200) {
            openNotificationWithIcon(
              "success",
              "Expense Setup",
              "Expense Setup Updated Successfully"
            )
            setOpen(false)
            setName("")
            setCheckExpense(false)
      ...
      CREATE("/api/Setup/CreateExpenseSetup", body)
        .then(res => {
          if (res.status == 200) {
            openNotificationWithIcon(
              "success",
              "Expense Setup",
              "Expense Setup Added Successfully"
            )
            setOpen(false)
            setCheckExpense(false)
            setName("")
  ...
  const editSetup = item => {
    ...
    setSetupId(item.id)
    setName(item.name)
    setComboVal(item.getAccFile3Setup ? item.getAccFile3Setup.id : null)
    setOpen(true)

    setCheckExpense(item.chkExpense)
    setIsEdit(true)

    setComboText(item.getAccFile3Setup ? item.getAccFile3Setup.name : null)
  }
  ...
  return (
    <React.Fragment>
      ...
      <div className="page-content">
        <Container fluid>
          {/* Render Breadcrumb */}
          <Breadcrumbs title="Admin" breadcrumbItem="Expense Setup" />
          <BackDrop open={show} />
                          ...
                          <TableBody>
                            {(rowsPerPage > 0
                              ...
                              <TableRow key={i}>
                                ...
                                <TableCell>
                                  {row.getAccFile3Setup
                                    ? row.getAccFile3Setup.name
                                    : "-"}
                                </TableCell>
                                <TableCell>
                                  <Button
        ...
        >
          <DialogTitle id="max-width-dialog-title">Expense Setup</DialogTitle>
          <DialogContent>
            <DialogContentText>
              <Form>
                <Row form>
                      ...
                      <FormControlLabel
                        control={
                          <Checkbox
                            ...
                            onChange={e => setCheckExpense(e.target.checked)}
                            name="checkedB"
                            color="primary"
                          />
                        }
