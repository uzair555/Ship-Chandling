...
export default function OPT_Type() {
  ...
  const upsertOptionType = () => {
    if (!!isEdit) {
      ...
      UPDATE(`/api/Setup/UpdateOptTypeSetup`, body)
        .then(res => {
          if (res.status == 200) {
      ...
      CREATE("/api/Setup/CreateOptTypeSetup", body)
        .then(res => {
          if (res.status == 200) {
            openNotificationWithIcon(
              "success",
              "Accounting Setup",
              "Option Type Added Successfully"
            )
            setOpen(false)
            setName("")
            setIsEdit(false)
  ...
  return (
    <React.Fragment>
      ...
      <div className="page-content">
        <Container fluid>
          {/* Render Breadcrumb */}
          <Breadcrumbs title="Admin" breadcrumbItem="Option Type Setup" />
          <BackDrop open={show} />
          <Row>
            <Col md="2"></Col>
            <Col lg="10">
            ...
            <DialogContentText>
              <Form>
                <Row form>
                  ...
                  <Col md={6}>
                    <FormGroup>
                      <Label for="exampleEmail">Option Type</Label>
                      <Input
                        type="text"
                        value={Name}
