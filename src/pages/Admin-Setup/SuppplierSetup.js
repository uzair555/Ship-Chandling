...
function SupplierSetup() {
  ...
  const getSetups = () => {
    setshow(true)
    GET(
      `/api/Setup/GetbySetupTypeMainSetup?SetupTypeId=73EC2F31-1AB4-45E3-9789-BF61C16BC040`
    )
      .then(setups => {
        if (setups.status == 200) {
          setRows(setups.data)
  ...
  const upsertSetup = () => {
    if (!!isEdit) {
      let body = {
        id: setupId,
        setupTypeId: "73EC2F31-1AB4-45E3-9789-BF61C16BC040",
        customerName: Name,
        regDate: regDate,
        emailAddress: Email,
        address: Address,
        ...
        .then(res => {
          if (res.status == 200) {
            openNotificationWithIcon(
              "success",
              "Supplier Setup",
              "Supplier Setup Updated Successfully"
            )
            setOpen(false)
            setIsEdit(false)
            resetVal()
        ...
        .then(res => {
          if (res.status == 200) {
            openNotificationWithIcon(
              "success",
              "Supplier Setup",
              "Supplier Setup Added Successfully"
            )
            setOpen(false)
            setIsEdit(false)
            resetVal()
  ...
  return (
    <React.Fragment>
      ...
      <div className="page-content">
        <Container fluid>
          {/* Render Breadcrumb */}
          <Breadcrumbs title="Admin" breadcrumbItem="Supplier Setup" />
          <BackDrop open={show} />
        ...
        >
          <DialogTitle id="max-width-dialog-title">Supplier Setup</DialogTitle>
          <DialogContent>
            <DialogContentText>
              <Form>
                <Row form>
