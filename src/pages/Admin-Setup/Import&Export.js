...
function ImportandExport() {
  ...
  const getSetups = () => {
    setshow(true)
    GET(
      `/api/Setup/GetbySetupTypeMainSetup?SetupTypeId=86EA916F-DE8F-4F1C-AA36-4086E0E8A507`
    )
      .then(setups => {
  ...
  const upsertSetup = () => {
    if (!!isEdit) {
      let body = {
        id: setupId,
        setupTypeId: "86EA916F-DE8F-4F1C-AA36-4086E0E8A507",
        customerName: Name,
        regDate: regDate,
        address: Address,
        openingBalance: 0,
        ...
        .then(res => {
          if (res.status == 200) {
            openNotificationWithIcon(
              "success",
              "Import / Export",
              "Import / Export Updated Successfully"
            )
            setOpen(false)
            setIsEdit(false)
            resetVal()
        ...
        .then(res => {
          if (res.status == 200) {
            openNotificationWithIcon(
              "success",
              "Import / Export",
              "Import / Export Added Successfully"
            )
            setOpen(false)
            setIsEdit(false)
            resetVal()
  ...
  const editSetup = item => {
    ...
    setAddress2(item.deliveryAddress)
  ...
  return (
    <React.Fragment>
      ...
      <div className="page-content">
        <Container fluid>
          {/* Render Breadcrumb */}
          <Breadcrumbs title="Admin" breadcrumbItem="Import & Export" />
          <BackDrop open={show} />
        ...
        >
          <DialogTitle id="max-width-dialog-title">Import / Export</DialogTitle>
          <DialogContent>
            <DialogContentText>
              <Form>
                <Row form>
