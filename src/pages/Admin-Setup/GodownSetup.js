...
export default function GoDownSetup() {
  ...
  const getGodownSetup = () => {
    setshow(true)
    GET("/api/Setup/GetAllGodownSetup")
      .then(setups => {
  ...
  const upsertGoDownSetup = () => {
    if (!!isEdit) {
      ...
      UPDATE(`/api/Setup/UpdateGodownSetup`, body)
        .then(res => {
          if (res.status == 200) {
      ...
      CREATE("/api/Setup/CreateGodownSetup", body)
        .then(res => {
          if (res.status == 200) {
            openNotificationWithIcon(
              "success",
  ...
  return (
    <React.Fragment>
      ...
      <div className="page-content">
        <Container fluid>
          {/* Render Breadcrumb */}
          <Breadcrumbs title="Admin" breadcrumbItem="Godown Setup" />
          <BackDrop open={show} />
          <Row>
            <Col md="2"></Col>
            <Col lg="10">
            ...
            <DialogContentText>
              <Form>
                <Row form>
                  ...
                  <Col md={6}>
                    <FormGroup>
                      <Label for="exampleEmail">Godown</Label>
                      <Input
                        type="text"
                        value={Name}
                        placeholder="Godown"
