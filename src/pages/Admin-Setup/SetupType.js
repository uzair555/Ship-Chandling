...
export default function SetupType() {
  ...
  const upsertSetupType = () => {
    if (!!isEdit) {
      ...
      UPDATE(`/api/Setup/UpdateSetupType`, body)
        .then(res => {
          if (res.status == 200) {
            openNotificationWithIcon(
              "success",
              "Setup Type",
              "Setup Type Updated Successfully"
            )
            setOpen(false)
            setName("")
            setIsEdit(false)
      ...
      CREATE("/api/Setup/CreateSetupType", body)
        .then(res => {
          if (res.status == 200) {
            openNotificationWithIcon(
              "success",
              "Setup Type",
              "Setup Type Added Successfully"
            )
            setOpen(false)
            setName("")
            setIsEdit(false)
  ...
  return (
    <React.Fragment>
      ...
      <div className="page-content">
        <Container fluid>
          {/* Render Breadcrumb */}
          <Breadcrumbs title="Admin" breadcrumbItem="Setup Type" />
          <BackDrop open={show} />
          <Row>
            <Col md="2"></Col>
            <Col lg="10">
        ...
        >
          <DialogTitle id="max-width-dialog-title">Setup Type</DialogTitle>
          <DialogContent>
            <DialogContentText>
              <Form>
                <Row form>
                  ...
                  <Col md={6}>
                    <FormGroup>
                      <Label for="exampleEmail">Setup Type</Label>
                      <Input
                        type="text"
                        value={Name}
