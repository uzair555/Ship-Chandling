...
function EmployeeRegistration() {
  ...
  const getSetups = () => {
    setshow(true)
    GET(
      `/api/Setup/GetbySetupTypeMainSetup?SetupTypeId=37FAE073-6DC0-4DF8-9F3D-DFD36701EC70`
    )
      .then(setups => {
        if (setups.status == 200) {
          setRows(setups.data)
  ...
  const upsertSetup = () => {
    if (!!isEdit) {
      let body = {
        id: setupId,
        setupTypeId: "37FAE073-6DC0-4DF8-9F3D-DFD36701EC70",
        customerName: Name,
        regDate: regDate,
        address: Address,
        ...
        .then(res => {
          if (res.status == 200) {
            openNotificationWithIcon(
              ...
              "Employee Registration Updated Successfully"
            )
            setOpen(false)
            setIsEdit(false)
            resetVal()
        ...
        .then(res => {
          if (res.status == 200) {
            openNotificationWithIcon(
              "success",
              "Employee Registration",
              "Employee Registration Added Successfully"
            )
            setOpen(false)
            setIsEdit(false)
            resetVal()
  ...
  return (
    <React.Fragment>
      ...
      <div className="page-content">
        <Container fluid>
          {/* Render Breadcrumb */}
          <Breadcrumbs title="Admin" breadcrumbItem="Employee Registration" />
          <BackDrop open={show} />
